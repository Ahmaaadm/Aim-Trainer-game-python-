...
LABEL_FONT = pygame.font.SysFont("cursive", 24)
LABEL_FONT_FINAL = pygame.font.SysFont("cursive",45) ## comicsans

sound_path = os.path.join("sounds", "../pistol.mp3")  # adjust the path to your MP3 sound file
sound = pygame.mixer.Sound(sound_path)

image_path = os.path.join("images", "../homani1.png")
image1 = pygame.image.load(image_path)

image_path2 = os.path.join("images", "../homani2.png")
image2 = pygame.image.load(image_path2)

sound_path2 = os.path.join("sounds", "../gameover.wav")  # adjust the path to your MP3 sound file
sound2 = pygame.mixer.Sound(sound_path2)

sound_pop_path = os.path.join("sounds","../popsound.mp3")  # adjust the path to your MP3 sound file
sound_pop = pygame.mixer.Sound(sound_pop_path)

game_music_path = os.path.join("sounds","../gamemusic.mp3")  # adjust the path to your MP3 sound file
game_music = pygame.mixer.Sound(game_music_path)

buzz_music_path = os.path.join("sounds","../buzz.wav")  # adjust the path to your MP3 sound file
buzz_music = pygame.mixer.Sound(buzz_music_path)

bloat_music_path = os.path.join("sounds","../bloatmessage.mp3")  # adjust the path to your MP3 sound file
bloat_music = pygame.mixer.Sound(bloat_music_path)
...
def end_screen(win, elapsed_time, target_pressed, clicks):
    ...
    ending_game_label = LABEL_FONT_FINAL.render(
        f"GAME OVER BLOAT -_-", 1, "red")
    ...
    message_label = LABEL_FONT.render("Press 'M' to hear your message", 1, "white")

    win.blit(ending_game_label, (get_middle(ending_game_label), 25))
    win.blit(time_label, (get_middle(time_label), 125))
    win.blit(speed_label, (get_middle(speed_label), 225))
    win.blit(hits_label, (get_middle(hits_label), 325))
    win.blit(accuracy_label, (get_middle(accuracy_label), 425))
    win.blit(restart_label, (get_middle(restart_label), 525))
    win.blit(message_label, (get_middle(message_label), 550))

    WIN.blit(image1, (-160, 0))
    WIN.blit(image2, (400, 0))
    sound2.play()
    pygame.display.flip()
...
def main():
    ...
    while run:
        clock.tick(70) # to slow the cpu work, or like an FPS adjustment
